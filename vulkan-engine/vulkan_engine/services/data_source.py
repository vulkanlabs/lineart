...
class DataSourceService(BaseService):
    ...
    def test_data_source(
        ...
        Args:
            data_source_id: Data source UUID
            configured_params: Runtime parameters for the request
            override_env_vars: Optional environment variables to override
            project_id: Optional project UUID to filter by

        Returns:
            Dictionary with test response data
        ...
        env_variables = (
            {ev.name: ev.value for ev in env_vars_list} if env_vars_list else {}
        )

        # Override with provided env vars
        if override_env_vars:
            env_variables.update(override_env_vars)

        # Check for missing variables (only if not overridden)
        ...
        if missing_vars:
            return {
                ...
                "cache_hit": False,
                "headers": {},
                "request_url": spec.source.url,
                "error_message": f"Missing environment variables: {', '.join(missing_vars)}",
            }

        # Check cache if enabled
        cache_hit = False
        if spec.caching and spec.caching.enabled:
            from vulkan_engine.data.broker import CacheManager, make_cache_key

            cache = CacheManager(
                self.db, self.logger.system if self.logger else None, spec
            )
            key = make_cache_key(spec, configured_params)
        ...
        try:
            ...
            req = make_request(spec.source, configured_params, env_variables)
            response = requests.Session().send(req, timeout=spec.source.timeout)
            response_time_ms = (time.time() - start_time) * 1000

            # Format response body based on content type
            response_body = None
            try:
                if spec.source.response_type == "JSON":
                    response_body = response.json()
                elif spec.source.response_type == "XML":
                    import xml.etree.ElementTree as ET

                    from vulkan_engine.data.broker import _xml_to_dict

                    response_body = _xml_to_dict(ET.fromstring(response.content))
                elif spec.source.response_type == "CSV":
                    import csv
                    import io

                    csv_reader = csv.DictReader(io.StringIO(response.text))
                    response_body = list(csv_reader)
                else:
    ...
    def update_data_source(
        ...
        """
        Update an existing data source (partial updates supported).

        Args:
            data_source_id: Data source UUID
            update_spec: Data source specification with fields to update
            project_id: Optional project UUID to filter by
        ...
        Raises:
            DataSourceNotFoundException: If data source doesn't exist
            DataSourceAlreadyExistsException: If updating name to existing name
        """
        ...
        if update_spec.name != data_source.name:
            if self.data_source_loader.data_source_exists(
                name=update_spec.name, project_id=project_id
            ):
                raise DataSourceAlreadyExistsException(
                    f"Data source with name {update_spec.name} already exists"
                )
            data_source.name = update_spec.name

        # Update fields
        if update_spec.description is not None:
            data_source.description = update_spec.description

        if update_spec.source:
            data_source.source = update_spec.source

        if update_spec.caching:
            data_source.caching = update_spec.caching

        if update_spec.metadata is not None:
            data_source.metadata = update_spec.metadata

        # Update variables and runtime params
        data_source.variables = update_spec.extract_env_vars()
        data_source.runtime_params = update_spec.extract_runtime_params()
