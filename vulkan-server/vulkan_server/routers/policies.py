...
async def create_policy(
    ...
    """Create a new policy."""
    return await service.create_policy(config)
...
async def update_policy(
    ...
    """Update a policy."""
    try:
        return await service.update_policy(policy_id, config)
...
async def list_policy_versions_by_policy(
    policy_id: str,
    include_archived: bool = False,
    service: PolicyService = Depends(get_policy_service),
):
...
async def list_runs_by_policy(
    ...
    runs = await service.list_runs_by_policy(policy_id, start_date_parsed, end_date_parsed)
    if not runs:
        return Response(status_code=204)
    return runs
...
async def create_run_group(
    ...
    try:
        return await service.create_run_group(policy_id, input_data, config_variables)
    except PolicyNotFoundException as e:
        raise HTTPException(status_code=404, detail=str(e))
    except InvalidAllocationStrategyException as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def run_duration_stats_by_policy(
    policy_id: str,
    ...
    return await service.get_run_duration_stats(policy_id, start_date, end_date, versions)


@router.post("/{policy_id}/runs/duration/by_status/query", response_model=list[Any])
async def run_duration_stats_by_policy_status(
    policy_id: str,
    start_date: datetime.date | None = None,
    end_date: datetime.date | None = None,
    versions: Annotated[list[str] | None, Body(embed=True)] = None,
    ...
    return await service.get_run_duration_by_status(policy_id, start_date, end_date, versions)


@router.post("/{policy_id}/runs/count/query", response_model=list[Any])
async def runs_by_policy(
    policy_id: str,
    start_date: datetime.date | None = None,
    end_date: datetime.date | None = None,
    versions: Annotated[list[str] | None, Body(embed=True)] = None,
    ...
    return await service.get_run_counts(policy_id, start_date, end_date, versions)


@router.post("/{policy_id}/runs/outcomes/query", response_model=list[Any])
async def runs_outcomes_by_policy(
    policy_id: str,
    ...
    return await service.get_run_outcomes(policy_id, start_date, end_date, versions)
