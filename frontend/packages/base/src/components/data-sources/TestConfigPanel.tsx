...
export const TestConfigPanel = forwardRef<TestConfigPanelRef, TestConfigPanelProps>(
    ({ dataSource, onTest, isLoading, initialConfig, onConfigChange }, ref) => {
                            ...
                            {runtimeParams.length === 0 && customParams.length === 0 && (
                                <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-md border border-dashed">
                                    ...
                                    <p className="text-sm text-muted-foreground">
                                        No runtime parameters configured. Click "Add Custom" to add
                                        test parameters.
                                    </p>
                                </div>
                            )}
                        ...
                        <div className="space-y-3">
                            {requiredVariables.map((varName) => (
                                <div key={varName}>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) =>
                                            handleEnvVarChange(varName, e.target.value)
                                        }
                            ...
                            {requiredVariables.length === 0 && customEnvVars.length === 0 && (
                                <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-md border border-dashed">
                                    <Info className="h-4 w-4 text-muted-foreground" />
                                    <p className="text-sm text-muted-foreground">
                                        No environment variables required. Click "Add Custom" to add
                                        test variables.
                                    </p>
                                </div>
                            )}
