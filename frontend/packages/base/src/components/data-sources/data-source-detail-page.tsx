...
import { DataTable, EnvironmentVariablesEditor, Loader } from "../..";
import type {
...
export interface DataSourceDetailPageConfig {
    ...
    fetchDataSourceEnvVars: (dataSourceId: string) => Promise<DataSourceEnvVarBase[]>;
    setDataSourceEnvVars: (dataSourceId: string, variables: DataSourceEnvVarBase[]) => Promise<void>;
    UsageAnalyticsComponent: React.ComponentType<{ dataSourceId: string }>;
}

export function DataSourceDetailPage({ config }: { config: DataSourceDetailPageConfig }) {
    const { dataSource, fetchDataSourceEnvVars, setDataSourceEnvVars, UsageAnalyticsComponent } = config;
    ...
    return (
        <div className="flex flex-col gap-6 p-6">
            ...
            >
                <UsageAnalyticsComponent dataSourceId={dataSource.data_source_id} />
            </Suspense>
        </div>
    );
}

function EditableVariablesCard({
    dataSource,
