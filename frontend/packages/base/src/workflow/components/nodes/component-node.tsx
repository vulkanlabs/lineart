...
import type { VulkanNodeData, VulkanNodeProps } from "@/workflow/types/workflow";
...
export function ComponentNode({ id, data, selected, height, width }: VulkanNodeProps) {
    ...
    // Get component data from providers
    ...
    const fieldMappings = metadata?.parameters || {};
        ...
        (fieldName: string, value: string) => {
            ...
            updateNodeData(id, {
                ...
                metadata: {
                    ...
                    parameters: newMappings,
                },
            });
        },
        [id, nodeData, fieldMappings, selectedComponent, updateNodeData],
    );
    ...
    // Set component ID in node data
    const setComponentID = useCallback(
        (component_id: string) => {
            updateNodeData(id, { ...(nodeData || {}), metadata: { component_id: component_id } });
        },
                ...
                {activeTab === "settings" && (
                    <div className="mt-4">
                        {!requiresAuth ? (
                            ...
                            <ComponentSettings serviceName="google" />
                        )}
                    </div>
                )}
            </div>
...
function ComponentSettings({ serviceName }: { serviceName: string }) {
    const { isAuthenticated, user, disconnect } = useServiceAuth(serviceName, null);

    return isAuthenticated ? (
        <div className="flex flex-col gap-2">
            <div className="flex flex-row gap-2 items-center">
                <p className="text-sm text-gray-800">
                    Authenticated as {user?.email || "your account"}
                </p>
            </div>
            <button
                onClick={disconnect}
