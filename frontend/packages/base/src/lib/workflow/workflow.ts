...
import { NodeLayoutConfig, NodeDefinition, EdgeLayoutConfig, Dict } from "./types";
import { NodeTypeMapping, layoutGraph, makeGraphElements } from "./graph";
...
export async function makeWorkflow(
    ...
    const modifiedEdges = edges.filter((edge: EdgeLayoutConfig) => {
        const fromChildOfClosedComponent =
            edge.fromComponentChild && edge.fromComponent && !componentsState[edge.fromComponent].isOpen;
        const toChildOfClosedComponent =
            edge.toComponentChild && edge.toComponent && !componentsState[edge.toComponent].isOpen;
