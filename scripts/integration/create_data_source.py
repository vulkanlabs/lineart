from vulkan.cli.client import component, data, policy, run
...
policy_id = policy.create_policy(
    ctx,
    name="Vendor Data",
    description="Vendor Data demo policy",
    input_schema="{}",
    output_schema="{}",
)
policy_version_id = policy.create_policy_version(
    ...
    policy_id=policy_id,
    version_name="v0.0.1",
    repository_path="examples/policies/data_input/",
)
...
policy_id = policy.create_policy(
    ctx,
    name="Vendor Data Comp",
    description="Vendor Data with component demo policy",
    input_schema="{}",
    output_schema="{}",
)
policy_version_id = policy.create_policy_version(
    ...
    policy_id=policy_id,
    version_name="v0.0.1",
    repository_path="examples/policies/data_input_comp/",
)
policy.set_active_version(ctx, policy_id=policy_id, policy_version_id=policy_version_id)
ctx.logger.info(f"Created policy {policy_id} with version {policy_version_id}")
